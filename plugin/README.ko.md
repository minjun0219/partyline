...
- **ack-after-inject.** 메시지는 세션의 로컬 소켓에 써진 뒤에만 ack(즉
  ...
  뒤에 무엇을 하는지(큐에 넣는지, 보여주는지, 버리는지)는 이 클라이언트가
  볼 수 없다. `partyline_status` 가 주입한 메시지 수와 스트림이 릴레이
  소식을 마지막으로 들은 시각을 보여준다.
...
3. 수신 세션 자체. 주입된 메시지는 그 세션의 입력이고, 입력을 어떻게
   다루는지는 그 세션의 몫이다 — 바쁜 턴이면 나중에 보이고, 클라이언트는
   소켓 너머를 볼 수 없다. 그때는 이미 릴레이에 ack 된 뒤라 메시지가
   되돌아오지 않는다; 수신 쪽 `/partyline:status` 의 `injected` 와
   `cursor` 가 거기까지 갔는지를 말해 준다.
